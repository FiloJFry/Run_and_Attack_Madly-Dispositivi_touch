...
{   
    ...
    AggiornaMirino();
    BottoneMischia.addEventListener('touchstart', (event) => {event.stopImmediatePropagation(); event.preventDefault(); if(!InPausa){BottoneMischia.style.opacity = "1"; setTimeout(() => {BottoneMischia.style.opacity = "0.5";},100);
    ...
    BottoneShotgun.addEventListener('touchstart',(event) => {event.stopImmediatePropagation(); event.preventDefault(); if(!InPausa){BottoneShotgun.style.opacity = "1"; setTimeout(() => {BottoneShotgun.style.opacity = "0.5";},100);
    ...
    BottoneAssalto.addEventListener('touchstart',(event) => {event.stopImmediatePropagation(); event.preventDefault(); if(!InPausa){BottoneAssalto.style.opacity = "1"; setTimeout(() => {BottoneAssalto.style.opacity = "0.5";},100);
            ...
            risparo = true;
            ArmaPresa = AssaltoEquipaggiato;}}});
    BottoneCecchino.addEventListener('touchstart',(event) => {event.stopImmediatePropagation(); event.preventDefault(); if(!InPausa){BottoneCecchino.style.opacity = "1"; setTimeout(() => {BottoneCecchino.style.opacity = "0.5";},100);
    ...
    BottoneRicarica.addEventListener('touchstart',(event) => {event.stopImmediatePropagation(); event.preventDefault(); if(!InPausa){BottoneRicarica.style.opacity = "1"; setTimeout(() => {BottoneRicarica.style.opacity = "0.5";},100);
            if(ArmaPresa != MischiaEquipaggiata && !InCarica)
            {   
                Pulisci();
                risparo = true;
                ArmaPresa.Ricarica();}}});
    BottoneSchiva.addEventListener('touchstart',(event) => {event.stopImmediatePropagation(); event.preventDefault(); if(!InPausa){BottoneSchiva.style.opacity = "1"; setTimeout(() => {BottoneSchiva.style.opacity = "0.5";},100);
                if(!Schivando && PuòSchivare)
                {  
                   Protagonista.Schiva(); 
}}});
    BottoneSpara.addEventListener('touchstart',(event) => {event.stopImmediatePropagation(); event.preventDefault(); if(!InPausa){BottoneSpara.style.opacity = "1";
    ...
    BottoneMuoviSu.addEventListener('touchstart',(event) => {event.stopImmediatePropagation(); event.preventDefault(); if(!InPausa){BottoneMuoviSu.style.opacity = "1";
                if(!Corri)
                {   
    ...
    BottoneMuoviGiù.addEventListener('touchstart',(event) => {event.stopImmediatePropagation(); event.preventDefault(); if(!InPausa){BottoneMuoviGiù.style.opacity = "1";
                if(!Corri)
                {
                    PersonaggioGiocabile.classList.add('Scuoti');
                    Corri = true;
    ...
    BottoneMuoviSu.addEventListener('touchend',(event) => {event.stopImmediatePropagation(); event.preventDefault(); if(!InPausa){BottoneMuoviSu.style.opacity = "0.5"; Corri = false;}});
    BottoneMuoviGiù.addEventListener('touchend',(event) => {event.stopImmediatePropagation(); event.preventDefault(); if(!InPausa){BottoneMuoviGiù.style.opacity = "0.5"; Corri = false;}});
    BottoneSpara.addEventListener('touchend',(event) => {event.stopImmediatePropagation(); event.preventDefault(); if(!InPausa){BottoneSpara.style.opacity = "0.5"; ArmaPresa.Arresta();}});
    document.addEventListener('touchstart',(event) => {event.stopImmediatePropagation(); event.preventDefault(); if(!InPausa){if(Spara != undefined){clearInterval(Spara); Spara = undefined;} document.querySelectorAll(".movimento , #Spara").forEach(B => {B.style.opacity = "0.5";}); PausaRiprendi(NemicoScelto)}});
    Partita = setInterval(() => {
